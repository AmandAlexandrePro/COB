...
play_death_vo(hit_location, player,mod,zombie)
{
	...
	if(hit_location == "head" && /*level.zombie_vars["zombie_insta_kill"] != 1*/ level.zombie_vars["zombie_insta_kill"] == 0 )
	{
	...
	if(weapon == "ray_gun" || weapon == "ray_gun_upgraded")
	{
		//Ray Gun Kills
...
//		}
		...
		structs = getstructarray( "initial_spawn_points", "targetname" ); // numan
			
		player = undefined;
		valid_player_found = false; 
		if( IsDefined( self.ignore_player ) )
		{
			players = array_remove( players, self.ignore_player );
		}
		while( !valid_player_found )
		{
			...
			if( !maps\_zombiemode_utility::is_player_valid( player ) || (isdefined(player.UsingZombieBlood) && player.UsingZombieBlood))
			{
