...
mapStartAudio()
{
	...
	// players[randomint(players.size)] thread level_start_vox(); //Plays a "Power's Out" Message from a random player at start !COB NO PLAY VOX START!
}
...
check_for_change()
{
	...
	while (1)
	{
		...
		if ( player GetStance() == "prone" )
		{
			...
			player maps\_zombiemode_score::add_to_player_score( level.random_change_found, true );
			play_sound_at_pos( "purchase", player.origin );
			break;
