...
init_weapons()
{
	add_zombie_weapon( "minigun_zm", 							"Press and Hold &&1 for Death Machine [Cost: 10000]",	10000,	"",	0 );
	// Pistols
	...
	add_zombie_weapon( "zombie_stg44", 							&"ZOMBIE_WEAPON_STG44_1200", 				1200,	"vox_mg",		0 );
	add_zombie_weapon( "zombie_stg44_upgraded", 				&"ZOMBIE_WEAPON_STG44_1200", 				1200,	"vox_mg",		0 );
...
{	
	if(isDefined(level.firesale) && level.firesale) // numan
	{
		self SetHintString( "Appuyez sur &&1 pour obtenir une arme aléatoire [Coût : 10]" );
	}
	else
	{
	...
	while( 1 )
	{
		...
		if(isDefined(user.UsingMinigun) && user.UsingMinigun) // numan
		{
			continue;
		}
		else if( is_player_valid( user ) && user.score >= level.zombie_treasure_chest_cost )
		{
			user maps\_zombiemode_score::minus_to_player_score( level.zombie_treasure_chest_cost ); 
			break; 
		}
		else if ( user.score < level.zombie_treasure_chest_cost )
		{
	...
	{
		player_size = getPlayers().size;
		// Let the player grab the weapon and re-enable the box //
		self.grab_weapon_hint = true;
		self.chest_user = user;
		if(player_size > 1)
			self sethintstring( "Appuyez et maintenez soit &&1 pour ^2prendre l'arme^7 ^1OU^7 soit [{+melee}] pour ^4partager l'arme^7" );
		else
			self sethintstring( "Appuyez et maintenez &&1 pour prendre l'arme" );
		self setCursorHint( "HINT_NOICON" );
		...
		if(player_size > 1)
			self thread chest_share(user);
		self chest_take(user, weapon_spawn_org);
		//self waittill("chest_take_or_share", grabber);
		/*while( 1 )
		{
			self waittill( "trigger", grabber ); 

			if(isDefined(grabber.UsingMinigun) && grabber.UsingMinigun)
			{
				continue;
			}
			...
			if( grabber == user || grabber == level )
			{
				...
				if( grabber == user && is_player_valid( user ) && user GetCurrentWeapon() != "mine_bouncing_betty" && (!isSubStr(user GetCurrentWeapon(), "zombie_item")) && level.falling_down == false )
				{
					bbPrint( "zombie_uses: playername %s playerscore %d round %d cost %d name %s x %f y %f z %f type magic_accept",
						user.playername, user.score, level.round_number, level.zombie_treasure_chest_cost, weapon_spawn_org.weapon_string, self.origin );
		...
		if(level.script != "nazi_zombie_prototype" && (!isdefined(level.firesale) || !level.firesale))
		{
...
chest_take(user, weapon_spawn_org)
{
	//self endon( "chest_take_or_share" );
	self waittill( "trigger", grabber );
	// COB TESTED NOT FULL
	if(!isDefined(grabber.UsingMinigun) || !grabber.UsingMinigun)
	{
		if( is_player_valid( grabber ) && grabber GetCurrentWeapon() != "mine_bouncing_betty" && (!isSubStr(grabber GetCurrentWeapon(), "zombie_item")) && level.falling_down == false )
		{
			bbPrint( "zombie_uses: playername %s playerscore %d round %d cost %d name %s x %f y %f z %f type magic_accept",
				grabber.playername, grabber.score, level.round_number, level.zombie_treasure_chest_cost, weapon_spawn_org.weapon_string, self.origin );
			self notify( "user_grabbed_weapon" );
...
chest_share(user)
{
	for(;;)
	{
		if(user IsTouching( self ) && user MeleeButtonPressed())
    	{
			self SetInvisibleToPlayer( user, true );
			self sethintstring( "Appuyez et maintenez &&1 pour prendre l'arme" );
			players = getPlayers();
			for(i=0;i<players.size;i++)
			{
				...
				if(grabbers != user)
    		    {
    		        self setvisibletoplayer( grabbers );
    		    }
			}
			break;
    	}
...
treasure_chest_weapon_spawn( chest, player )
{
	...
	if(level.script != "nazi_zombie_prototype" && getdvar("magic_chest_movable") == "1")
	{
		...
		if(!isDefined(level.firesale) || !level.firesale) // numan
		{
			...
			{
				if(!isdefined(chest.isFireSaleBox) || !chest.isFireSaleBox)
				{
