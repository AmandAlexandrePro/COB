...
vending_upgrade()
{
	...
	for( ;; )
	{
		...
		if( player HasWeapon(current_weapon) && player HasWeapon(current_weapon + "_upgraded"))
		{
			continue;
		}
...
revive_machine_exit()
{
	...
	machine_clip = GetEnt( "clip_revive", "targetname" );
...
/*	machine_song = GetEnt( "perksacola", "targetname" );
	...
	machine_clip MoveTo( machine_clip.origin + ( 0, 0, 32 ), 5 );
...
vending_set_hintstring( perk )
{
	...
	case "specialty_quickrevive":
		if(get_players().size == 1)
			self SetHintString( "Appuyez et maintenez &&1 pour acheter Réanimation rapide [Coût : 500]" );
		else
			self SetHintString( &"Appuyez et maintenez &&1 pour acheter Réanimation rapide [Coût : 1500]" );
		break;
